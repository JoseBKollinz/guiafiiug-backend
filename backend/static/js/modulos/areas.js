...
async function renderAreas() {
  ...
  try {
    const areas = await window.fetchConCache("/api/areas-comunes", { method: "GET" });
    ...
    cont.innerHTML = areas.map(a => `
      <div class="item-card">
        <div class="item-card-main">
          <div class="item-card-icon"><i class="ti ti-map-pin"></i></div>
        ...
        <div class="item-card-actions">
          ...
          <button data-eliminar="${a.id}"><i class="ti ti-trash"></i></button>
        </div>
      </div>
    `).join("");
...
async function guardarArea() {
  ...
  try {
    ...
    if (res.ok) {
      cerrarModal();
      await window.fetchConCache("/api/areas-comunes", { method: "GET" }, true);
      renderAreas();
    } else {
